...
DECLARE
    ...
    v_compra_id BIGINT;  -- Alterado para BIGINT combinando com o tipo real de compra.id
    v_venda_id BIGINT;   -- Alterado para BIGINT combinando com o tipo real de venda.id
    v_descarte_id INT;
    v_cliente_id INT;
    v_user_id BIGINT;    -- Alterado para BIGINT combinando com o tipo real de usuario.id
...
BEGIN
    ...
    FOR v_produto_record IN SELECT id, preco_venda FROM produto LOOP
        -- Removida a coluna "data" da tabela compra de forma definitiva
    ...
    FOR v_dia IN REVERSE 89..1 LOOP
        FOR v_venda_no IN 1..(2 + FLOOR(RANDOM() * 4)) LOOP
            ...
            -- Removida coluna morta "data" da tabela venda. Desconto mapeado como booleano puro (false).
            ...
            FOR v_item IN 1..(1 + FLOOR(RANDOM() * 3)) LOOP
                ...
                IF v_produto_record.unidade_medida = 'UN' THEN v_quantidade := 1.00; ELSE v_quantidade := ROUND((0.600 + (RANDOM() * 1.500))::NUMERIC, 3); END IF;
    ...
    INSERT INTO venda (data_venda, valor_total, descuento, fk_usuario_id, fk_cliente_id, created_at, updated_at)
    VALUES (NOW(), ROUND((9.960 * 69.90)::NUMERIC, 2), false, v_user_id, NULL, NOW(), NOW()) RETURNING id INTO v_venda_id;
