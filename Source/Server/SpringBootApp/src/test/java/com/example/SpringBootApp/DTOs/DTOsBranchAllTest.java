...
class DTOsBranchAllTest {

    @Test
    void exerciseEqualsHashCodeAndInequalityForManyDTOs() throws Exception {
        String[] classes = {
        ...
        for (String className : classes) {
            ...
            assertNotNull(a, "Could not instantiate " + className);
            assertNotNull(b, "Could not instantiate " + className);

            setDefaultsOnFields(cls, a);
            setDefaultsOnFields(cls, b);

            // equality and hashcode
            assertEquals(a, b, "Objects should be equal: " + className);
            assertEquals(a.hashCode(), b.hashCode(), "Hashcodes should match: " + className);
            ...
            for (Field f : fields) {
                ...
                if (diff == null) {
                    if (!f.getType().isPrimitive()) {
                        try { f.set(b, null); changed = true; } catch (Exception e) { /* skip */ }
                    }
                } else {
                    try { f.set(b, diff); changed = true; } catch (IllegalArgumentException | IllegalAccessException ex) {
                        // try setter
